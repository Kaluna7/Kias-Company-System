// app/stores/AuditProgram/lpStore.js
import { create } from "zustand";

const API_BASE = "/api/AuditProgram/lp";

export const useLpStore = create((set, get) => ({
  data: [],
  loading: false,
  error: null,
  ...
  fetchLpData: async ({ page = 1, pageSize = 50, q = "", status, year } = {}) => {
  ...
  createLpAp: async (payload) => {
    ...
    try {
      ...
      const json = await res.json();
      await get().fetchLpData();
      set({ loading: false });
      return json;
    } catch (err) {
      console.error("LP create error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
  ...
  moveToDraft: async (risk_id) => {
    ...
    try {
      ...
      console.error("LP moveToDraft error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
  ...
  moveToPublish: async (risk_id) => {
    ...
    try {
      ...
      console.error("LP moveToPublish error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
