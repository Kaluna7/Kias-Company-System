// app/stores/AuditProgram/misStore.js
import { create } from "zustand";

const API_BASE = "/api/AuditProgram/mis";
...
export const useMisStore = create((set, get) => ({
  ...
  fetchMisData: async ({ page = 1, pageSize = 50, q = "", status, year } = {}) => {
  ...
  createMisAp: async (payload) => {
    ...
    try {
      ...
      const json = await res.json();
      await get().fetchMisData();
      set({ loading: false });
      return json;
    } catch (err) {
      console.error("MIS create error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
  ...
  moveToDraft: async (risk_id) => {
    ...
    try {
      ...
      console.error("MIS moveToDraft error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
  ...
  moveToPublish: async (risk_id) => {
    ...
    try {
      ...
      console.error("MIS moveToPublish error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
