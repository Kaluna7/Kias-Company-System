// app/stores/AuditProgram/opsStore.js
import { create } from "zustand";

const API_BASE = "/api/AuditProgram/ops";
...
export const useOpsStore = create((set, get) => ({
  ...
  fetchOpsData: async ({ page = 1, pageSize = 50, q = "", status, year } = {}) => {
  ...
  moveToDraft: async (risk_id) => {
    ...
    try {
      ...
      console.error("Ops moveToDraft error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
  ...
  moveToPublish: async (risk_id) => {
    ...
    try {
      ...
      console.error("Ops moveToPublish error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
