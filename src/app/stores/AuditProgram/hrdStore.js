// app/stores/AuditProgram/hrdStore.js
import { create } from "zustand";

const API_BASE = "/api/AuditProgram/hrd";
...
export const useHrdStore = create((set, get) => ({
  ...
  fetchHrdData: async ({ page = 1, pageSize = 50, q = "", status, year } = {}) => {
    ...
    try {
      ...
      console.error("HRD fetch error:", err);
  ...
  createHrdAp: async (payload) => {
    ...
    try {
      ...
      console.error("HRD create error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
  ...
  moveToDraft: async (risk_id) => {
    ...
    try {
      ...
      console.error("HRD moveToDraft error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
  ...
  moveToPublish: async (risk_id) => {
    ...
    try {
      ...
      console.error("HRD moveToPublish error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
