// app/stores/AuditProgram/gaStore.js
import { create } from "zustand";

const API_BASE = "/api/AuditProgram/ga";

export const useGaStore = create((set, get) => ({
  data: [],
  loading: false,
  error: null,
  ...
  fetchGaData: async ({ page = 1, pageSize = 50, q = "", status, year } = {}) => {
  ...
  createGaAp: async (payload) => {
    ...
    try {
      ...
      const json = await res.json();
      await get().fetchGaData();
      set({ loading: false });
      return json;
    } catch (err) {
      console.error("GA create error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
  ...
  moveToDraft: async (risk_id) => {
    ...
    try {
      ...
      console.error("GA moveToDraft error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
  ...
  moveToPublish: async (risk_id) => {
    ...
    try {
      ...
      console.error("GA moveToPublish error:", err);
      set({ loading: false, error: err.message ?? String(err) });
      throw err;
    }
  },
