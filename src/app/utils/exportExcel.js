...
export function exportToStyledExcel(
  ...
  // Nama sheet & file
  const sheetName = typeof columns === "string" ? columns : componentName;
  ...
  const titleLines = [
    ...
    `RISK ASSESSMENT FORM (${componentName})`,
  ...
  titleLines.forEach((line, idx) => {
    ...
    if (worksheet[cellAddress]) {
      worksheet[cellAddress].s =
        idx === 0 ? titleStyle : idx === 1 ? subtitleStyle : dateStyle;
    }
  });
  ...
  worksheet["!cols"] = normalizedCols.map((col) => ({
    wch: Math.max(12, col.header.length + 10),
  }));
    ...
    for (let c = range.s.c; c <= range.e.c; c++) {
      ...
      if (worksheet[addr]) {
        ...
        if (headerName.toLowerCase().includes("priority")) {
          ...
          if (!isNaN(val)) {
            if (val <= 2)
              worksheet[addr].s.fill = { fgColor: { rgb: "C6EFCE" } }; // hijau
            else if (val > 6)
              worksheet[addr].s.fill = { fgColor: { rgb: "FFC7CE" } }; // merah
