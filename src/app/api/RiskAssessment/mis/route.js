...
export async function GET(req) {
  try {
    ...
    const [miss, total] = await Promise.all([
      prisma.mis.findMany({
        where,
      ...
      prisma.mis.count({ where: where ?? {} }),
    ]);

    const safeMis = await backfillRiskIdNoForRows(prisma.mis, miss);

    if (includeAps) {
      const flattened = safeMis.map((p) => {
    ...
    return new Response(JSON.stringify({ data: safeMis, meta: { total, page, pageSize } }), { status: 200 });
  } catch (err) {
...
export async function POST(req) {
  try {
    ...
    const created = await prisma.mis.create({
      data: {
    ...
    const risk_id_no = await ensureRiskIdNo(prisma.mis, created.risk_id, created.risk_id_no);
    return new Response(JSON.stringify({ ...created, risk_id_no }), { status: 201 });
  } catch (err) {
    console.error("POST /api/mis error:", err);
    return new Response(
      JSON.stringify({ error: err.message ?? "Server error" }),
      { status: 500 },
    );
