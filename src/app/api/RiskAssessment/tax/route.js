...
export async function GET(req) {
  try {
    ...
    const [taxs, total] = await Promise.all([
      prisma.tax.findMany({
        where,
    ...
    return new Response(JSON.stringify({ data: safeTax, meta: { total, page, pageSize } }), { status: 200 });
  } catch (err) {
    console.error("GET /api/taxt error:", err);
    return new Response(
