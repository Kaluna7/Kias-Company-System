...
export async function PUT(req, { params }) {
  try {
    ...
    const existing = await prisma.tax.findUnique({ where: { risk_id: id } });
    if (!existing) {
    ...
    console.error("PUT /api/tax/[id] error:", err);
...
export async function DELETE(req, { params }) {
  try {
    ...
    await resequenceGapRiskIdNoForApiPath(prisma, "tax");

    return new Response(JSON.stringify({ message: "Tax record deleted successfully" }), { status: 200 });
  } catch (err) {
    console.error("DELETE /api/RiskAssessment/tax/[id] error:", err);
    return new Response(JSON.stringify({ error: err.message ?? "Server error" }), { status: 500 });
  }
}