...
export async function GET(req) {
  try {
    ...
    const [storeDesigns, total] = await Promise.all([
      prisma.sdp.findMany({
        where,
      ...
      prisma.sdp.count({ where: where ?? {} }),
    ]);

    const safeSdp = await backfillRiskIdNoForRows(prisma.sdp, storeDesigns);

    if (includeAps) {
      const flattened = safeSdp.map((p) => {
    ...
    return new Response(JSON.stringify({ data: safeSdp, meta: { total, page, pageSize } }), { status: 200 });
  } catch (err) {
    console.error("GET /api/sdp error:", err);
    return new Response(
...
export async function POST(req) {
  try {
    ...
    const created = await prisma.sdp.create({
      data: {
    ...
    const risk_id_no = await ensureRiskIdNo(prisma.sdp, created.risk_id, created.risk_id_no);
    return new Response(JSON.stringify({ ...created, risk_id_no }), { status: 201 });
  } catch (err) {
    console.error("POST /api/sdp error:", err);
    return new Response(
      JSON.stringify({ error: err.message ?? "Server error" }),
      { status: 500 },
    );
