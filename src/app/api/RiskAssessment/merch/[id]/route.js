...
export async function PUT(req, { params }) {
  try {
    ...
    const existing = await prisma.merchandise.findUnique({ where: { risk_id: id } });
    if (!existing) {
    ...
    console.error("PUT /api/merch/[id] error:", err);
...
export async function DELETE(req, { params }) {
  try {
    ...
    await resequenceGapRiskIdNoForApiPath(prisma, "merch");

    return new Response(JSON.stringify({ message: "Merchandise record deleted successfully" }), { status: 200 });
  } catch (err) {
    console.error("DELETE /api/RiskAssessment/merch/[id] error:", err);
    return new Response(JSON.stringify({ error: err.message ?? "Server error" }), { status: 500 });
  }
}