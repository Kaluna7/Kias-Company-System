...
export async function PUT(req, { params }) {
  try {
    ...
    const existing = await prisma.warehouse.findUnique({ where: { risk_id: id } });
    if (!existing) {
    ...
    console.error("PUT /api/whs/[id] error:", err);
...
export async function DELETE(req, { params }) {
  try {
    ...
    await resequenceGapRiskIdNoForApiPath(prisma, "whs");

    return new Response(JSON.stringify({ message: "Warehouse record deleted successfully" }), { status: 200 });
  } catch (err) {
    console.error("DELETE /api/RiskAssessment/whs/[id] error:", err);
    return new Response(JSON.stringify({ error: err.message ?? "Server error" }), { status: 500 });
  }
}