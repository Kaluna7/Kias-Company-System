...
export async function GET(req) {
  try {
    ...
    if (includeAps) {
      const flattened = safeOperationals.map((p) => {
    ...
    return new Response(JSON.stringify({ data: safeOperationals, meta: { total, page, pageSize } }), { status: 200 });
  } catch (err) {
...
export async function POST(req) {
  try {
    ...
    const risk_id_no = await ensureRiskIdNo(prisma.operational, created.risk_id, created.risk_id_no);
