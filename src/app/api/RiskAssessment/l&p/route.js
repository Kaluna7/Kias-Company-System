...
export async function GET(req) {
  try {
    ...
    const [lossPreventions, total] = await Promise.all([
      prisma.lp.findMany({
        where,
      ...
      prisma.lp.count({ where: where ?? {} }),
    ]);

    const safeLp = await backfillRiskIdNoForRows(prisma.lp, lossPreventions);

    if (includeAps) {
      const flattened = safeLp.map((p) => {
    ...
    return new Response(JSON.stringify({ data: safeLp, meta: { total, page, pageSize } }), { status: 200 });
  } catch (err) {
...
export async function POST(req) {
  try {
    ...
    const created = await prisma.lp.create({
      data: {
    ...
    const risk_id_no = await ensureRiskIdNo(prisma.lp, created.risk_id, created.risk_id_no);
    return new Response(JSON.stringify({ ...created, risk_id_no }), { status: 201 });
  } catch (err) {
    console.error("POST /api/LossPrevention error:", err);
    return new Response(
      JSON.stringify({ error: err.message ?? "Server error" }),
      { status: 500 },
    );
