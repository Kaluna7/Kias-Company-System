...
export async function GET(req) {
  try {
    ...
    const where = { department: "ACCOUNTING" };
...
export async function POST(req) {
  try {
    ...
    const created = await prisma.worksheet_finance.create({
      data: {
        department: body.department || "ACCOUNTING",
  ...
  try {
    ...
    if (latest) {
      ...
      await prisma.worksheet_finance.create({
        data: { department: "ACCOUNTING", status_wp: body.statusWP !== undefined && body.statusWP !== "" ? body.statusWP : null },
      });
    }
