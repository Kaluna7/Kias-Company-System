...
export async function GET(req) {
  try {
    ...
    const where = buildWorksheetFindManyWhere("TAX", url.searchParams);
    ...
    console.error("GET /api/worksheet/tax error:", err);
    return new Response(
...
export async function POST(req) {
  try {
    ...
    const data = await executeWorksheetPost(prisma, "TAX", body, publish);
    return new Response(
    ...
    console.error("POST /api/worksheet/tax error:", err);
    return new Response(
      JSON.stringify({ success: false, error: err.message ?? "Server error" }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
...
export async function PATCH(req) {
  try {
    ...
    const body = await req.json();
    const url = new URL(req?.url || "", "http://localhost");
    await executeWorksheetPatch(prisma, "TAX", body, url.searchParams.get("year"));
