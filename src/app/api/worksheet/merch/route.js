...
export async function GET(req) {
  try {
    ...
    const where = buildWorksheetFindManyWhere("MERCHANDISE", url.searchParams);
    ...
    console.error("GET /api/worksheet/merch error:", err);
    return new Response(
...
export async function POST(req) {
  try {
    ...
    const data = await executeWorksheetPost(prisma, "MERCHANDISE", body, publish);
    return new Response(
    ...
    console.error("POST /api/worksheet/merch error:", err);
    return new Response(
      JSON.stringify({ success: false, error: err.message ?? "Server error" }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
...
export async function PATCH(req) {
  try {
    ...
    const body = await req.json();
    const url = new URL(req?.url || "", "http://localhost");
    await executeWorksheetPatch(prisma, "MERCHANDISE", body, url.searchParams.get("year"));
    return new Response(JSON.stringify({ success: true }), { status: 200, headers: { "Content-Type": "application/json" } });
  } catch (err) {
    console.error("PATCH /api/worksheet/merch error:", err);
