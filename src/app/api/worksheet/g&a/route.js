...
export async function POST(req) {
  try {
    ...
    const created = await prisma.worksheet_finance.create({
      data: {
        department: body.department || "G&A",
  ...
  try {
    ...
    if (latest) {
      ...
      await prisma.worksheet_finance.create({
        data: { department: "G&A", status_wp: body.statusWP !== undefined && body.statusWP !== "" ? body.statusWP : null },
      });
    }
