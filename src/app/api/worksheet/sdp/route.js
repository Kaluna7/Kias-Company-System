...
export async function GET(req) {
  try {
    ...
    const where = { department: "DESIGN STORE PLANNER" };
    ...
    console.error("GET /api/worksheet/sdp error:", err);
    return new Response(
...
export async function PATCH(req) {
  try {
    ...
    const latest = await prisma.worksheet_finance.findFirst({
      where: { department: "DESIGN STORE PLANNER" },
      orderBy: { created_at: "desc" },
    });
    if (latest) {
      await prisma.worksheet_finance.update({
      ...
      await prisma.worksheet_finance.create({
        data: { department: "DESIGN STORE PLANNER", status_wp: body.statusWP !== undefined && body.statusWP !== "" ? body.statusWP : null },
      });
    }
