...
export async function PUT(req) {
  try {
    ...
    const existing = await prisma.financeAp.findUnique({ where: { ap_id } });
    if (!existing) {
      return NextResponse.json({ error: `AP with ap_id=${ap_id} not found` }, { status: 404 });
    }
    if (existing.finance_risk_id !== finance_risk_id) {
      return NextResponse.json({ error: "ap_id does not belong to this finance_risk_id" }, { status: 400 });
    }

    const updated = await prisma.financeAp.update({
      where: { ap_id },
      data: {
        substantive_test: body.substantive_test ?? null,
        objective: body.objective ?? null,
    ...
    console.error("PUT /api/AuditProgram/finance/ap error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
