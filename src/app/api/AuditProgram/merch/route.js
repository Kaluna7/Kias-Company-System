...
async function buildNextApCode(merchandise_risk_id, baseCode) {
  const siblings = await prisma.merchandiseAp.findMany({
    where: { merchandise_risk_id },
    select: { ap_code: true },
  });
...
export async function GET(req) {
  try {
    ...
    const parents = await prisma.merchandise.findMany({
      where,
      include: { aps: true },
    ...
    console.error("GET /api/AuditProgram/merch error:", err);
...
export async function POST(req) {
  try {
    ...
    const baseCode = (parent.risk_id_no || `MERCH-${parent.risk_id}`).trim();
    const autoApCode = await buildNextApCode(merchandise_risk_id, baseCode);
    const requestedApCode = typeof body.ap_code === "string" ? body.ap_code.trim() : "";
    const ap_code = requestedApCode || autoApCode;

    const created = await prisma.merchandiseAp.create({
      data: {
        ap_code,
        ...
        merchandise: { connect: { risk_id: merchandise_risk_id } },
      },
    });

    return NextResponse.json({ data: created }, { status: 201 });
  } catch (err) {
    console.error("POST /api/AuditProgram/merch error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
