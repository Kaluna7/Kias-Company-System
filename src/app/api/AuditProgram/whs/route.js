...
async function buildNextApCode(warehouse_risk_id, baseCode) {
  const siblings = await prisma.warehouseAp.findMany({
    where: { warehouse_risk_id },
    select: { ap_code: true },
  });
...
export async function GET(req) {
  try {
    ...
    const parents = await prisma.warehouse.findMany({
      where,
      include: { aps: true },
    ...
    console.error("GET /api/AuditProgram/whs error:", err);
...
export async function POST(req) {
  try {
    ...
    const baseCode = (parent.risk_id_no || `WHS-${parent.risk_id}`).trim();
    const autoApCode = await buildNextApCode(warehouse_risk_id, baseCode);
    const requestedApCode = typeof body.ap_code === "string" ? body.ap_code.trim() : "";
    const ap_code = requestedApCode || autoApCode;

    const created = await prisma.warehouseAp.create({
      data: {
        ap_code,
        ...
        warehouse: { connect: { risk_id: warehouse_risk_id } },
      },
    });

    return NextResponse.json({ data: created }, { status: 201 });
  } catch (err) {
    console.error("POST /api/AuditProgram/whs error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
