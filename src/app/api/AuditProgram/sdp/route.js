...
async function buildNextApCode(sdp_risk_id, baseCode) {
  const siblings = await prisma.sdpAp.findMany({
    where: { sdp_risk_id },
    select: { ap_code: true },
  });
...
export async function GET(req) {
  try {
    ...
    const parents = await prisma.sdp.findMany({
      where,
      include: { aps: true },
    ...
    const total = await prisma.sdp.count({ where });
    ...
    console.error("GET /api/AuditProgram/sdp error:", err);
...
export async function POST(req) {
  try {
    ...
    const sdp_risk_id = Number(body.sdp_risk_id || 0);
    if (!sdp_risk_id) {
      return NextResponse.json({ error: "sdp_risk_id is required" }, { status: 400 });
    }

    const parent = await prisma.sdp.findUnique({ where: { risk_id: sdp_risk_id } });
    if (!parent) {
      return NextResponse.json({ error: `SDP parent with risk_id=${sdp_risk_id} not found` }, { status: 400 });
    }

    const baseCode = (parent.risk_id_no || `SDP-${parent.risk_id}`).trim();
    const autoApCode = await buildNextApCode(sdp_risk_id, baseCode);
    const requestedApCode = typeof body.ap_code === "string" ? body.ap_code.trim() : "";
    const ap_code = requestedApCode || autoApCode;

    const created = await prisma.sdpAp.create({
      data: {
        ap_code,
        ...
        sdp: { connect: { risk_id: sdp_risk_id } },
      },
    });

    return NextResponse.json({ data: created }, { status: 201 });
  } catch (err) {
    console.error("POST /api/AuditProgram/sdp error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
...
export async function PUT(req) {
  try {
    ...
    const existing = await prisma.sdpAp.findUnique({ where: { ap_id } });
    if (!existing) {
      return NextResponse.json({ error: `AP with ap_id=${ap_id} not found` }, { status: 404 });
    }
    if (existing.sdp_risk_id !== sdp_risk_id) {
      return NextResponse.json({ error: "ap_id does not belong to this sdp_risk_id" }, { status: 400 });
    }

    const updated = await prisma.sdpAp.update({
      where: { ap_id },
      data: {
        substantive_test: body.substantive_test ?? null,
        objective: body.objective ?? null,
    ...
    console.error("PUT /api/AuditProgram/sdp error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
