...
async function buildNextApCode(lp_risk_id, baseCode) {
  const siblings = await prisma.lpAp.findMany({
    where: { lp_risk_id },
    select: { ap_code: true },
  });
...
export async function GET(req) {
  try {
    ...
    const parents = await prisma.lp.findMany({
      where,
      include: { aps: true },
    ...
    const total = await prisma.lp.count({ where });
    ...
    console.error("GET /api/AuditProgram/lp error:", err);
...
export async function POST(req) {
  try {
    ...
    const lp_risk_id = Number(body.lp_risk_id || 0);
    if (!lp_risk_id) {
      return NextResponse.json({ error: "lp_risk_id is required" }, { status: 400 });
    }

    const parent = await prisma.lp.findUnique({ where: { risk_id: lp_risk_id } });
    if (!parent) {
      return NextResponse.json({ error: `LP parent with risk_id=${lp_risk_id} not found` }, { status: 400 });
    }

    const baseCode = (parent.risk_id_no || `LP-${parent.risk_id}`).trim();
    const autoApCode = await buildNextApCode(lp_risk_id, baseCode);
    const requestedApCode = typeof body.ap_code === "string" ? body.ap_code.trim() : "";
    const ap_code = requestedApCode || autoApCode;

    const created = await prisma.lpAp.create({
      data: {
        ap_code,
        ...
        lp: { connect: { risk_id: lp_risk_id } },
      },
    });

    return NextResponse.json({ data: created }, { status: 201 });
  } catch (err) {
    console.error("POST /api/AuditProgram/lp error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
...
export async function PUT(req) {
  try {
    ...
    const existing = await prisma.lpAp.findUnique({ where: { ap_id } });
    if (!existing) {
      return NextResponse.json({ error: `AP with ap_id=${ap_id} not found` }, { status: 404 });
    }
    if (existing.lp_risk_id !== lp_risk_id) {
      return NextResponse.json({ error: "ap_id does not belong to this lp_risk_id" }, { status: 400 });
    }

    const updated = await prisma.lpAp.update({
      where: { ap_id },
      data: {
        substantive_test: body.substantive_test ?? null,
        objective: body.objective ?? null,
    ...
    console.error("PUT /api/AuditProgram/lp error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
