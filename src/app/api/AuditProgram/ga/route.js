...
async function buildNextApCode(general_affair_risk_id, baseCode) {
  const siblings = await prisma.generalAffairAp.findMany({
    where: { general_affair_risk_id },
    select: { ap_code: true },
  });
...
export async function GET(req) {
  try {
    ...
    const parents = await prisma.general_affair.findMany({
      where,
      include: { aps: true },
    ...
    const total = await prisma.general_affair.count({ where });
    ...
    console.error("GET /api/AuditProgram/ga error:", err);
...
export async function POST(req) {
  try {
    ...
    const baseCode = (parent.risk_id_no || `GA-${parent.risk_id}`).trim();
    const autoApCode = await buildNextApCode(general_affair_risk_id, baseCode);
    const requestedApCode = typeof body.ap_code === "string" ? body.ap_code.trim() : "";
    const ap_code = requestedApCode || autoApCode;

    const created = await prisma.generalAffairAp.create({
      data: {
        ap_code,
        ...
        general_affair: { connect: { risk_id: general_affair_risk_id } },
      },
    });

    return NextResponse.json({ data: created }, { status: 201 });
  } catch (err) {
    console.error("POST /api/AuditProgram/ga error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
