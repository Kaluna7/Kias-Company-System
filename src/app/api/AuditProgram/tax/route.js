...
async function buildNextApCode(tax_risk_id, baseCode) {
  const siblings = await prisma.taxAp.findMany({
    where: { tax_risk_id },
    select: { ap_code: true },
  });
...
export async function GET(req) {
  try {
    ...
    const parents = await prisma.tax.findMany({
      where,
      include: { aps: true },
    ...
    console.error("GET /api/AuditProgram/tax error:", err);
...
export async function POST(req) {
  try {
    ...
    const tax_risk_id = Number(body.tax_risk_id || 0);
    if (!tax_risk_id) {
      return NextResponse.json({ error: "tax_risk_id is required" }, { status: 400 });
    }

    const parent = await prisma.tax.findUnique({ where: { risk_id: tax_risk_id } });
    if (!parent) {
      return NextResponse.json({ error: `Tax parent with risk_id=${tax_risk_id} not found` }, { status: 400 });
    }

    const baseCode = (parent.risk_id_no || `TAX-${parent.risk_id}`).trim();
    const autoApCode = await buildNextApCode(tax_risk_id, baseCode);
    const requestedApCode = typeof body.ap_code === "string" ? body.ap_code.trim() : "";
    const ap_code = requestedApCode || autoApCode;

    const created = await prisma.taxAp.create({
      data: {
        ap_code,
        ...
        tax: { connect: { risk_id: tax_risk_id } },
      },
    });

    return NextResponse.json({ data: created }, { status: 201 });
  } catch (err) {
    console.error("POST /api/AuditProgram/tax error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
...
export async function PUT(req) {
  try {
    ...
    const existing = await prisma.taxAp.findUnique({ where: { ap_id } });
    if (!existing) {
      return NextResponse.json({ error: `AP with ap_id=${ap_id} not found` }, { status: 404 });
    }
    if (existing.tax_risk_id !== tax_risk_id) {
      return NextResponse.json({ error: "ap_id does not belong to this tax_risk_id" }, { status: 400 });
    }

    const updated = await prisma.taxAp.update({
      where: { ap_id },
      data: {
        substantive_test: body.substantive_test ?? null,
        objective: body.objective ?? null,
    ...
    console.error("PUT /api/AuditProgram/tax error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
