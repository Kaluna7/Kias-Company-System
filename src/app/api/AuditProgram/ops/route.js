...
async function buildNextApCode(operational_risk_id, baseCode) {
  const siblings = await prisma.operationalAp.findMany({
    where: { operational_risk_id },
    select: { ap_code: true },
  });
...
export async function GET(req) {
  try {
    ...
    const parents = await prisma.operational.findMany({
      where,
      include: { aps: true },
    ...
    const total = await prisma.operational.count({ where });
    ...
    console.error("GET /api/AuditProgram/ops error:", err);
...
export async function POST(req) {
  try {
    ...
    const operational_risk_id = Number(body.operational_risk_id || 0);
    if (!operational_risk_id) {
      return NextResponse.json({ error: "operational_risk_id is required" }, { status: 400 });
    }

    const parent = await prisma.operational.findUnique({ where: { risk_id: operational_risk_id } });
    if (!parent) {
      return NextResponse.json({ error: `Operational parent with risk_id=${operational_risk_id} not found` }, { status: 400 });
    }

    const baseCode = (parent.risk_id_no || `OPS-${parent.risk_id}`).trim();
    const autoApCode = await buildNextApCode(operational_risk_id, baseCode);
    const requestedApCode = typeof body.ap_code === "string" ? body.ap_code.trim() : "";
    const ap_code = requestedApCode || autoApCode;

    const created = await prisma.operationalAp.create({
      data: {
        ap_code,
        ...
        operational: { connect: { risk_id: operational_risk_id } },
      },
    });

    return NextResponse.json({ data: created }, { status: 201 });
  } catch (err) {
    console.error("POST /api/AuditProgram/ops error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
...
export async function PUT(req) {
  try {
    ...
    const existing = await prisma.operationalAp.findUnique({ where: { ap_id } });
    if (!existing) {
      return NextResponse.json({ error: `AP with ap_id=${ap_id} not found` }, { status: 404 });
    }
    if (existing.operational_risk_id !== operational_risk_id) {
      return NextResponse.json({ error: "ap_id does not belong to this operational_risk_id" }, { status: 400 });
    }

    const updated = await prisma.operationalAp.update({
      where: { ap_id },
      data: {
        substantive_test: body.substantive_test ?? null,
        objective: body.objective ?? null,
    ...
    console.error("PUT /api/AuditProgram/ops error:", err);
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
}
