...
function HRDWorksheetPageContent() {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (file) {
      try {
        ...
        formData.append("department", "hrd");
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const response = await fetch("/api/worksheet/hrd", {
        method: "POST",
        headers: {
  ...
  return (
    <main className="flex flex-col w-full h-screen overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="flex flex-col flex-1 w-full h-full overflow-hidden">
        <SmallHeader label="B.1.3 WORKSHEET - HRD" showSearch={false} backHref={`/Page/worksheet${yearParam ? `?year=${encodeURIComponent(yearParam)}` : ""}`} />
...
export default function HRDWorksheet() {
  return (
