...
const API = "/api/worksheet/ops";

function OpsWorksheetPageContent() {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (file) {
      try {
        ...
        formData.append("department", "ops");
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const response = await fetch("/api/worksheet/ops", {
        method: "POST",
        headers: {
  ...
  return (
    <main className="flex flex-col w-full h-screen overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="flex flex-col flex-1 w-full h-full overflow-hidden">
        <SmallHeader label="B.1.10 WORKSHEET - OPERATIONAL" showSearch={false} backHref={`/Page/worksheet${yearParam ? `?year=${encodeURIComponent(yearParam)}` : ""}`} />
...
export default function OpsWorksheet() {
  return (
    <Suspense fallback={<div className="p-4 text-sm text-gray-500">Loading...</div>}>
      <OpsWorksheetPageContent />
    </Suspense>
  );
}
