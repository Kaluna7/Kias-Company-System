...
export default function SdpClient({ initialData, initialSortBy = "risk_id_no", initialSortDir = "asc" }) {
  ...
  const handleSubmitSdpAp = async (sdp_risk_id, payload) => {
    ...
    <main className="flex h-dvh max-h-dvh w-full flex-col overflow-hidden bg-slate-50">
            ...
            <DataTableAudit
              ...
              onDeleteAp={async (apRow, deptApi) => {
                try {
                  ...
                  await fetchSdpData({ q: "", page: 1, pageSize: 50, status: viewDraft ? "draft" : "published", year: yearParam || undefined });
                } catch (err) {
      ...
      {showModal.open && (showModal.mode === "add-ap" || showModal.mode === "edit-ap") && (
        <GenericInputModal
          ...
          onSubmit={(payload) =>
            handleSubmitSdpAp(showModal.selectedRow?.risk_id, {
              ...payload,
