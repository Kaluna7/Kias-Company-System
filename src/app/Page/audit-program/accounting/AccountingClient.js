...
import { useAccountingStore } from "@/app/stores/AuditProgram/accountingStore";
...
export default function AccountingClient({ initialData, initialSortBy = "risk_id_no", initialSortDir = "asc" }) {
  const {
    data,
    loading,
    error,
    ...
    return [
      {
        ...
        action: async () => {
          setViewDraft(true);
          await fetchAccountingData({ q: "", page: 1, pageSize: 50, status: "draft", year: yearParam || undefined });
        },
      },
      {
        name: "View Published",
        action: async () => {
          setViewDraft(false);
          await fetchAccountingData({ q: "", page: 1, pageSize: 50, status: "published", year: yearParam || undefined });
        },
      },
    ];
  }, [isAdmin, fetchAccountingData]);
  ...
  return (
    <main className="flex flex-col w-full h-screen overflow-hidden">
      <div className="flex flex-col flex-1 w-full h-full">
        <SmallHeader
          label={`Accounting Audit Program - ${viewDraft ? "Draft Data" : "Published Data"}`}
