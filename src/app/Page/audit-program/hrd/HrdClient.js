...
import { useHrdStore } from "@/app/stores/AuditProgram/hrdStore";
...
export default function HrdClient({ initialData, initialSortBy = "risk_id_no", initialSortDir = "asc" }) {
    ...
    return [
      {
        ...
        action: async () => {
          setViewDraft(true);
          await fetchHrdData({ q: "", page: 1, pageSize: 50, status: "draft", year: yearParam || undefined });
        },
      },
      {
        name: "View Published",
        action: async () => {
          setViewDraft(false);
          await fetchHrdData({ q: "", page: 1, pageSize: 50, status: "published", year: yearParam || undefined });
        },
      },
    ];
  }, [isAdmin, fetchHrdData]);
  ...
  const handleSubmitHrdAp = async (hrd_risk_id, payload) => {
    const isEditAp = Boolean(payload?.ap_id);
    const res = await fetch("/api/AuditProgram/hrd/", {
  ...
  return (
    <main className="flex flex-col w-full h-screen overflow-hidden">
      <div className="flex flex-col flex-1 w-full h-full">
        <SmallHeader
          label={`HRD Audit Program - ${viewDraft ? "Draft Data" : "Published Data"}`}
            ...
            <DataTableAudit
              ...
              onDeleteAp={async (apRow, deptApi) => {
                try {
                  ...
                  await fetchHrdData({ q: "", page: 1, pageSize: 50, status: viewDraft ? "draft" : "published", year: yearParam || undefined });
                } catch (err) {
      ...
      {showModal.open && (showModal.mode === "add-ap" || showModal.mode === "edit-ap") && (
        <GenericInputModal
          ...
          onSubmit={(payload) =>
            handleSubmitHrdAp(showModal.selectedRow?.risk_id, {
              ...payload,
