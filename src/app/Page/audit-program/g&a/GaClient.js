...
import { useGaStore } from "@/app/stores/AuditProgram/gaStore";
...
export default function GaClient({ initialData, initialSortBy = "risk_id_no", initialSortDir = "asc" }) {
  const {
    data,
    loading,
    error,
    ...
    return [
      {
        ...
        action: async () => {
          setViewDraft(true);
          await fetchGaData({ q: "", page: 1, pageSize: 50, status: "draft", year: yearParam || undefined });
        },
      },
      {
        name: "View Published",
        action: async () => {
          setViewDraft(false);
          await fetchGaData({ q: "", page: 1, pageSize: 50, status: "published", year: yearParam || undefined });
        },
      },
    ];
  }, [isAdmin, fetchGaData]);
  ...
  const handleSubmitGaAp = async (general_affair_risk_id, payload) => {
    const isEditAp = Boolean(payload?.ap_id);
    const res = await fetch("/api/AuditProgram/ga/", {
      method: isEditAp ? "PUT" : "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        general_affair_risk_id,
        ...payload,
      }),
    });
    if (!res.ok) {
  ...
  return (
    <main className="flex h-dvh max-h-dvh w-full flex-col overflow-hidden bg-slate-50">
      <SmallHeader
          label={`General Affair Audit Program - ${viewDraft ? "Draft Data" : "Published Data"}`}
            ...
            <DataTableAudit
              ...
              onDeleteAp={async (apRow, deptApi) => {
                try {
                  ...
                  await fetchGaData({ q: "", page: 1, pageSize: 50, status: viewDraft ? "draft" : "published", year: yearParam || undefined });
                } catch (err) {
      ...
      {showModal.open && (showModal.mode === "add-ap" || showModal.mode === "edit-ap") && (
        <GenericInputModal
          ...
          onSubmit={(payload) =>
            handleSubmitGaAp(showModal.selectedRow?.risk_id, {
              ...payload,
