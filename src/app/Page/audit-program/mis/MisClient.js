...
import { useMisStore } from "@/app/stores/AuditProgram/misStore";
...
export default function MisClient({ initialData, initialSortBy = "risk_id_no", initialSortDir = "asc" }) {
  const {
    data,
    loading,
    error,
    ...
    return [
      {
        ...
        action: async () => {
          setViewDraft(true);
          await fetchMisData({ q: "", page: 1, pageSize: 50, status: "draft", year: yearParam || undefined });
        },
      },
      {
        name: "View Published",
        action: async () => {
          setViewDraft(false);
          await fetchMisData({ q: "", page: 1, pageSize: 50, status: "published", year: yearParam || undefined });
        },
      },
    ];
  }, [isAdmin, fetchMisData]);
  ...
  const handleSubmitMisAp = async (mis_risk_id, payload) => {
    const isEditAp = Boolean(payload?.ap_id);
    const res = await fetch("/api/AuditProgram/mis/", {
    ...
    <main className="flex h-dvh max-h-dvh w-full flex-col overflow-hidden bg-slate-50">
            ...
            <DataTableAudit
              ...
              onDeleteAp={async (apRow, deptApi) => {
                try {
                  ...
                  await fetchMisData({ q: "", page: 1, pageSize: 50, status: viewDraft ? "draft" : "published", year: yearParam || undefined });
                } catch (err) {
      ...
      {showModal.open && (showModal.mode === "add-ap" || showModal.mode === "edit-ap") && (
        <GenericInputModal
          ...
          onSubmit={(payload) =>
            handleSubmitMisAp(showModal.selectedRow?.risk_id, {
              ...payload,
