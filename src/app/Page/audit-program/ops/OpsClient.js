...
export default function OpsClient({ initialData, initialSortBy = "risk_id_no", initialSortDir = "asc" }) {
    ...
    return [
      {
        ...
        action: async () => {
          setViewDraft(true);
          await fetchOpsData({ q: "", page: 1, pageSize: 50, status: "draft" });
        },
      },
      {
        name: "View Published",
        action: async () => {
          setViewDraft(false);
          await fetchOpsData({ q: "", page: 1, pageSize: 50, status: "published" });
        },
      },
    ];
  ...
  const handleSubmitNewOpsAp = async (operational_risk_id, payload) => {
    ...
    <main className="flex flex-col w-full h-screen overflow-hidden">
      ...
      {showModal.open && showModal.mode === "add-ap" && (
        <GenericInputModal
          ...
          onSubmit={(payload) =>
            handleSubmitNewOpsAp(showModal.selectedRow?.risk_id, payload)
          }
          listForm={listFormAp}
