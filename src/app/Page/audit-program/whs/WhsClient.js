...
import { useWhsStore } from "@/app/stores/AuditProgram/whsStore";
...
export default function WhsClient({ initialData, initialSortBy = "risk_id_no", initialSortDir = "asc" }) {
  const {
    data,
    loading,
    error,
    ...
    return [
      {
        ...
        action: async () => {
          setViewDraft(true);
          await fetchWhsData({ q: "", page: 1, pageSize: 50, status: "draft", year: yearParam || undefined });
        },
      },
      {
        name: "View Published",
        action: async () => {
          setViewDraft(false);
          await fetchWhsData({ q: "", page: 1, pageSize: 50, status: "published", year: yearParam || undefined });
        },
      },
    ];
  }, [isAdmin, fetchWhsData]);
  ...
  const handleSubmitWhsAp = async (warehouse_risk_id, payload) => {
    const isEditAp = Boolean(payload?.ap_id);
    const res = await fetch("/api/AuditProgram/whs/", {
  ...
  return (
    <main className="flex h-dvh max-h-dvh w-full flex-col overflow-hidden bg-slate-50">
      <SmallHeader
          label={`Warehouse Audit Program - ${viewDraft ? "Draft Data" : "Published Data"}`}
            ...
            <DataTableAudit
              ...
              onDeleteAp={async (apRow, deptApi) => {
                try {
                  ...
                  await fetchWhsData({ q: "", page: 1, pageSize: 50, status: viewDraft ? "draft" : "published", year: yearParam || undefined });
                } catch (err) {
      ...
      {showModal.open && (showModal.mode === "add-ap" || showModal.mode === "edit-ap") && (
        <GenericInputModal
          ...
          onSubmit={(payload) =>
            handleSubmitWhsAp(showModal.selectedRow?.risk_id, {
              ...payload,
