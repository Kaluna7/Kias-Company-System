...
export default function MisClient({ initialData = [], initialMeta = null }) {
  ...
  const setMisAndMeta = useMisStore((s) => s.setMisAndMeta);
  ...
  useEffect(() => {
    ...
    if (initialData?.length > 0 || initialMeta) {
      setMisAndMeta(initialData ?? [], initialMeta);
    }
  }, [initialData, initialMeta, setMisAndMeta]);
  ...
  const items = useMemo(() => {
    ...
    if (isAdmin) {
      ...
      base.push({
        name: "Export Data",
        action: () => {
          const mis = useMisStore.getState().mis;
          if (!mis || mis.length === 0) {
          ...
          exportToStyledExcel(mis, null, status, "MIS");
        },
  ...
  function MisTableWrapper() {
    const meta = useMisStore((s) => s.meta);
    const load = useCallback(
      async (page) => {
        ...
        try {
          await loadMis(viewDraft ? "draft" : "published", page ?? meta?.page ?? 1, meta?.pageSize ?? 50);
        } finally {
          isLoadingRef.current = false;
          setPaginationLoading(false);
        }
      },
      [loadMis, viewDraft, meta?.page, meta?.pageSize]
    );
  ...
  return (
    <main className="flex flex-col w-full h-screen overflow-hidden">
      <div className="flex flex-col flex-1 w-full h-full">
        <SmallHeader
          label="Risk Assessment Form MIS"
          fileItems={items}
