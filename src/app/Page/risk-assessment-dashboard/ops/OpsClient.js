...
export default function OpsClient({ initialData = [], initialMeta = null }) {
  ...
  const setOpsAndMeta = useOperationalStore((s) => s.setOpsAndMeta);
  ...
  useEffect(() => {
    ...
    if (initialData?.length > 0 || initialMeta) {
      setOpsAndMeta(initialData ?? [], initialMeta);
    }
  }, [initialData, initialMeta, setOpsAndMeta]);

  const skipLoadForPublishedRef = useRef(!!(initialData?.length > 0));
  const meta = useOperationalStore((s) => s.meta);
  ...
  const loadPage = useCallback(
    ...
    [loadOperational, viewDraft, yearParam]
  );
    ...
    return [
      {
        ...
        action: async () => {
          ...
          try {
            ...
            setEditMode(false);
            await loadOperational("draft", 1, 50, yearParam || undefined);
          } finally {
            isLoadingRef.current = false;
          }
        },
      },
      {
        ...
        action: async () => {
          ...
          try {
            ...
            setConvertMode(false);
            setEditMode(false);
            await loadOperational("published", 1, 50, yearParam || undefined);
          } finally {
            isLoadingRef.current = false;
          }
        },
  ...
  }, [isAdmin, loadOperational, yearParam]);
          ...
          {isOpen && (
            <NewOperationalInput
              onClose={() => {
                closePopUp();
                setSelectedItem(null);
                loadOperational(viewDraft ? "draft" : "published", 1, 50, yearParam || undefined);
              }}
