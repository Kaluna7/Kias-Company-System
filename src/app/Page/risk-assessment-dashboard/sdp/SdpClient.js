...
export default function SdpClient({ initialData = [], initialMeta = null }) {
  ...
  const loadPage = useCallback(
    ...
    [loadStorePlanning, viewDraft, yearParam]
  );
    ...
    return [
      {
        ...
        action: async () => {
          ...
          try {
            ...
            setEditMode(false);
            await loadStorePlanning("draft", 1, 50, yearParam || undefined);
          } finally {
            isLoadingRef.current = false;
          }
        },
      },
      {
        ...
        action: async () => {
          ...
          try {
            ...
            setConvertMode(false);
            setEditMode(false);
            await loadStorePlanning("published", 1, 50, yearParam || undefined);
          } finally {
            isLoadingRef.current = false;
          }
        },
  ...
  }, [isAdmin, loadStorePlanning, yearParam]);
          ...
          {isOpen && (
            <NewSDPInput
              onClose={() => {
                closePopUp();
                setSelectedItem(null);
                loadStorePlanning(viewDraft ? "draft" : "published", 1, 50, yearParam || undefined);
              }}
