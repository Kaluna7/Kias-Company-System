...
export default function MerchClient({ initialData = [], initialMeta = null }) {
  ...
  const setMerchAndMeta = useMerchandiseStore((s) => s.setMerchAndMeta);
  ...
  useEffect(() => {
    ...
    if (initialData?.length > 0 || initialMeta) {
      setMerchAndMeta(initialData ?? [], initialMeta);
    }
    ...
    return [
      {
        ...
        action: async () => {
          ...
          try {
            ...
            setEditMode(false);
            await loadMerchandise("draft", 1, 50, yearParam || undefined);
          } finally {
            isLoadingRef.current = false;
          }
        },
      },
      {
        ...
        action: async () => {
          ...
          try {
            ...
            setConvertMode(false);
            setEditMode(false);
            await loadMerchandise("published", 1, 50, yearParam || undefined);
          } finally {
            isLoadingRef.current = false;
          }
        },
          ...
          {isOpen && (
            <NewMerchandiseInput
              onClose={() => {
                closePopUp();
                setSelectedItem(null);
                loadMerchandise(viewDraft ? "draft" : "published", 1, 50, yearParam || undefined);
              }}
