...
export default function LpClient({ initialData = [], initialMeta = null }) {
  ...
  const setLpAndMeta = useLossPreventionStore((s) => s.setLpAndMeta);
  ...
  useEffect(() => {
    ...
    if (initialData?.length > 0 || initialMeta) {
      setLpAndMeta(initialData ?? [], initialMeta);
    }
  }, [initialData, initialMeta, setLpAndMeta]);
  ...
  const items = useMemo(() => {
    ...
    if (isAdmin) {
      ...
      base.push({
        name: "Export Data",
        action: () => {
          const lossPreventions = useLossPreventionStore.getState().lp;
          if (!lossPreventions || lossPreventions.length === 0) {
          ...
          exportToStyledExcel(lossPreventions, null, status, "L&P");
        },
    ...
    return [
      {
        ...
        action: async () => {
          ...
          try {
            ...
            setEditMode(false);
            await loadLossPrevention("draft");
          } finally {
      ...
      },
      {
        ...
        action: async () => {
          ...
          try {
            ...
            setConvertMode(false);
            setEditMode(false);
            await loadLossPrevention("published");
          } finally {
            isLoadingRef.current = false;
          }
        },
  ...
  function LossPreventionTableWrapper() {
    const meta = useLossPreventionStore((s) => s.meta);
    const load = useCallback(
      async (page) => {
        ...
        try {
          await loadLossPrevention(viewDraft ? "draft" : "published", page ?? meta?.page ?? 1, meta?.pageSize ?? 50);
        } finally {
          isLoadingRef.current = false;
          setPaginationLoading(false);
        }
      },
      [loadLossPrevention, viewDraft, meta?.page, meta?.pageSize]
    );
    ...
    const sortedLossPreventions = useMemo(() => {
      if (!sortOption || !lossPreventions) return lossPreventions;

      return [...lossPreventions].sort((a, b) => {
    ...
    }, [lossPreventions, sortOption]);

    return (
      <>
        <DataTable
          apiPath="l&p"
          items={sortedLossPreventions}
  ...
  return (
    <main className="flex flex-col w-full h-screen overflow-hidden">
      <div className="flex flex-col flex-1 w-full h-full">
        <SmallHeader
          label="Risk Assessment Form L&P"
          fileItems={items}
          ...
          {isOpen && (
            <NewLpInput
              onClose={() => {
                closePopUp();
                setSelectedItem(null);
                loadLossPrevention(viewDraft ? "draft" : "published");
              }}
              defaultData={selectedItem}
            />
          )}
          <LossPreventionTableWrapper />
        </div>
      </div>
    </main>
  );
