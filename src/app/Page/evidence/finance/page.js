...
export default function FinanceEvidence() {
  ...
  // Only fetch once on mount, not on every session change
  useEffect(() => {
    ...
    const fetchApData = async () => {
      try {
        ...
        const riskResponse = await fetch("/api/RiskAssessment/finance?status=published&includeAps=true");
        ...
        const evidenceResponse = await fetch("/api/evidence/finance?department=FINANCE");
        ...
        if (riskData && Array.isArray(riskData) && riskData.length > 0) {
          ...
          console.log("Total data from Risk Assessment:", riskData.length);
          console.log("Data with Risk ID NO (AP Code):", apDataWithCodes.length);
          console.log("Sample data:", riskData.slice(0, 3));
          ...
          if (evidenceResult.success && evidenceResult.data && Array.isArray(evidenceResult.data)) {
            evidenceResult.data.forEach((ev) => {
              // Map by ap_code (which stores risk_id_no)
              if (ev.ap_code) {
          ...
          // Transform data to include evidence fields
          ...
          // Set preparer from session if not set, otherwise use saved evidence
          ...
          console.warn("No data returned from Risk Assessment API or data is empty");
          setApData([]);
        }
      } catch (error) {
  ...
  const handleFileUpload = async (index, e) => {
    ...
    try {
      ...
      const response = await fetch("/api/evidence/finance", {
        method: "POST",
        body: formData,
      });
      ...
      if (result.success) {
        // Update the row with file information
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 max-w-[95%] mx-auto border border-gray-200">
            ...
            <div className="mb-6 space-y-2">
              ...
              <div className="flex flex-col gap-1 text-sm text-gray-700">
                <p><span className="font-semibold">DEPARTMENT:</span> FINANCE</p>
                <p><span className="font-semibold">PREPARER:</span> {preparer}</p>
              </div>
            </div>
                    ...
                    <tr>
                      <td colSpan="5" className="border border-gray-300 px-4 py-8 text-center text-gray-500">
                        <div className="space-y-3">
                          ...
                          <p className="text-sm">Please create risk data in <strong>Risk Assessment Dashboard → Finance</strong> first.</p>
